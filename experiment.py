...
flags.DEFINE_integer(
    "total_environment_frames", int(1e9), "Total environment frames to train for."
)
...
flags.DEFINE_string("level_name", "PongNoFrameskip-v4", "Name of OpenAI gym task.")
...
## Env seed patch for the Atari environment.
flags.DEFINE_integer("env_seed", None, "Seed for the environment.")
flags.DEFINE_integer("test_seed", None, "Seed for testing the environment.")

timestamp = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S-%f")
run_name = f"{FLAGS.level_name}-{timestamp}"

flags.DEFINE_string("run_name", run_name, "Name of the run.")
save_dir = str(Path(FLAGS.logdir) / FLAGS.run_name)
...
def create_environment(level_name, seed, is_test=False, env_seed=None, logdir=None):
    """Creates an environment wrapped in a `FlowEnvironment`."""
    p = py_process.PyProcess(
        environments.PyProcessAtari, FLAGS.level_name, seed, env_seed, logdir
    )
...
def train(action_set, level_names):
    ...
    with tf.Graph().as_default():
        ...
        env = create_environment(
            level_names[0], seed=1, env_seed=FLAGS.env_seed, logdir=save_dir
        )
    ...
    with tf.Graph().as_default(), tf.device(
        ...
        for i in range(FLAGS.num_actors):
            if is_actor_fn(i):
                ...
                env = create_environment(
                    level_name, seed=i + 1, env_seed=FLAGS.env_seed, logdir=save_dir
                )
        ...
        with tf.train.MonitoredTrainingSession(
            ...
            if is_learner:
                ...
                summary_writer = tf.summary.FileWriterCache.get(save_dir)

                tf.logging.info(
                    "Starting run for invocation '%s' on %s",
                    " ".join(sys.argv),
...
def test(action_set, level_names):
    ...
    with tf.Graph().as_default():
        ...
        for level_name in level_names:
            # TODO add test seed input treatment
            env = create_environment(
                level_name,
                seed=1,
                is_test=True,
                env_seed=FLAGS.test_seed or None,
                logdir=save_dir,
            )
            outputs[level_name] = build_actor(agent, env, level_name, action_set)

        with tf.train.SingularMonitoredSession(
            checkpoint_dir=save_dir, hooks=[py_process.PyProcessHook()]
        ) as session:
            for level_name in level_names:
